...
import com.ifsp.todolistarq.databinding.FragmentTarefaBinding
...
class TarefaFragment : BaseFragment() {
        ...
        if (tarefaExtra != null) {
            ...
            with(fragmentTarefaBinding) {
                ...
                realizadaTarefaCb.isChecked =
                    tarefaExtra.realizada != 0 // Zero é falso porque Sqlite não tem Boolean
            }
    ...
    override fun onStart() {
        ...
        requireActivity().registerReceiver(
            receiveInserirTarefasBr, IntentFilter(
                ACTION_INSERIR
            )
        )

        requireActivity().registerReceiver(
            receiveAtualizarTarefasBr, IntentFilter(
                ACTION_ATUALIZAR
            )
        )
    }
